---
title: "User Manuals - Helios Vision AI (EN)"
---

This set of manuals describes the use of the Customer Portal by module. Each document is independent and written in Spanish (ASCII) for initial review.

## How to use these manuals

- Each module has its own file.
- Button and label names are left as they appear in the UI (usually in English).
- Each manual includes: objective, access, roles, requirements, steps, fields and data, good practices, and related links.
- Suggestions for illustrations (captures) are included where they add value.

## Common roles

- owner: full control of the tenant.
- admin: module administration and configuration.
- agent: daily operation (conversations, inbox, agents).
- viewer: read only.

## Index

- [00-overview.md](./00-overview.md)
- [01-authentication.md](./01-authentication.md)
- [02-dashboard.md](./02-dashboard.md)
- [03-agents.md](./03-agents.md)
- [04-inbox-email.md](./04-inbox-email.md)
- [05-whatsapp.md](./05-whatsapp.md)
- [06-voice-agents.md](./06-voice-agents.md)
- [07-web-chat.md](./07-web-chat.md)
- [08-integrations.md](./08-integrations.md)
- [09-tools.md](./09-tools.md)
- [10-documents.md](./10-documents.md)
- [11-data-tables.md](./11-data-tables.md)
- [12-contacts.md](./12-contacts.md)
- [13-analytics.md](./13-analytics.md)
- [14-team.md](./14-team.md)
- [15-settings-general.md](./15-settings-general.md)
- [16-settings-categories.md](./16-settings-categories.md)
- [17-settings-gmail.md](./17-settings-gmail.md)
- [18-settings-api-keys.md](./18-settings-api-keys.md)
- [19-billing-usage.md](./19-billing-usage.md)
- [20-notifications.md](./20-notifications.md)
- [21-onboarding-plan.md](./21-onboarding-plan.md)
- [22-twilio-setup.md](./22-twilio-setup.md)
- [23-web-chat-widget.md](./23-web-chat-widget.md)
- [99-capturas.md](./99-capturas.md)